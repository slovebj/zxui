<template>
  <div class="c-slider">
    ...
    <div class="c-slider-runway"
      :class="{ 'show-input': showInput }"
      @click="onSliderClick" ref="slider">
      <div class="c-slider-bar" :style="{ width: currentPosition }"></div>
      <div class="c-slider-button-wrapper" @mouseenter="hovering = true" @mouseleave="hovering = false" :style="{left: currentPosition}" ref="button">
        <div class="c-slider-button" :class="{ 'hover': hovering, 'dragging': dragging }"></div>
      </div>
      <transition name="popper-fade">
        <div class="c-slider-pop" v-show="showTip" ref="pop">{{ value }}</div>
      </transition>
      <div class="c-slider-stop" v-for="item in stops" :style="{ 'left': item + '%' }" v-if="showStops"></div>
    </div>
  </div>
</template>
